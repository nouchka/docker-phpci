...
if [ ! -d "/var/www/phpci/public/" ]; then
	...
	php ./console phpci:install --url=$PHPCI_URL --db-host=$MYSQL_SERVER --db-name=$MYSQL_DBB --db-user=$MYSQl_USER --db-pass=$MYSQl_PASSWORD --admin-name=$ADMIN_NAME --admin-pass=$ADMIN_PASS --admin-mail=$ADMIN_MAIL
